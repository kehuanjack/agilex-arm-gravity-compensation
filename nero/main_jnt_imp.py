...
def main():
    ...
    urdf_path = str(PROJECT_ROOT / "nero" / "nero" / "urdf" / "nero_description.urdf")
    ...
    # 每个关节单独设置阻尼 b 和刚度 k（按 J1~J7 顺序）
    b = np.array([0.5, 0.8, 0.8, 0.8, 0.2, 0.2, 0.2], dtype=float)
    k = np.array([10.0, 10.0, 10.0, 10.0, 1.0, 1.0, 1.0], dtype=float)
    ...
    try:
        ...
        print("\n用户中断，停止关节阻抗控制")
        robot.move_j(joint_angles.tolist())
